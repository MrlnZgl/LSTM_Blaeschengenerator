// aktualisierung des LED Status wenn Taster gedrückt funktioniert nicht --> morgen bearbeiten 
  ...
  if (digitalRead(button) == HIGH) { // Button pressed
      delay(50); // debounce delay
       if (digitalRead(button) == HIGH) { // Check again after debounce
          ...
          while (digitalRead(button) == HIGH); // Wait for button to be released
      }
  }
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            // Füge den aktuellen LED-Status hinzu
        ...
        if (currentLine.endsWith("GET /H")) {
          ...
          ledState = true;              // GET /H turns the LED on
        }
        if (currentLine.endsWith("GET /L")) {
          digitalWrite(led, LOW); 
          ledState = false;               // GET /L turns the LED off
        }
      }
    }
