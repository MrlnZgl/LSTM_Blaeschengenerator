...
AccelStepper myStepper(AccelStepper::FULL4WIRE, motorPins[0], motorPins[1], motorPins[2], motorPins[3]);
...
float motorSpeed; // in U/min
float motorSteps; // in 1/sec
...
void setup() {
    ...
    motorSpeed = float(spindleRotationalSpeed / 60.0); // in U/min
    motorSteps = (motorSpeed * stepsPerRevolution) / 60.0;
    ...
    myStepper.setMaxSpeed(motorSteps); // Maximalgeschwindigkeit in Schritten pro Sekunde
    myStepper.setAcceleration(500); // Beschleunigung in Schritten pro Sekunde^2
...
void sendMotorControl(WiFiClient &client) {
...
void sendDiagram(WiFiClient &client){
    // Canvas für das Diagramm
    client.println("<canvas id='volumeChart' width='400' height='200' style='border:1px solid #000000;'></canvas>");

    // JavaScript zur Erstellung und Aktualisierung des Diagramms
    client.println("<script>");
    client.println("let volumeData = [];");   // Array für das Volumen
    client.println("let timeData = [];");     // Array für die Zeitstempel
    client.println("let canvas = document.getElementById('volumeChart');");
    client.println("let ctx = canvas.getContext('2d');");

    // Funktion zum Zeichnen des Liniendiagramms
    client.println("function drawChart() {");
    client.println("  ctx.clearRect(0, 0, canvas.width, canvas.height);"); // Lösche den vorherigen Inhalt
    client.println("  ctx.beginPath();");
    client.println("  ctx.moveTo(0, canvas.height - volumeData[0]);"); // Setze den Startpunkt");
    client.println("  for (let i = 1; i < volumeData.length; i++) {");
    client.println("    let x = (canvas.width / volumeData.length) * i;");
    client.println("    let y = canvas.height - volumeData[i];"); // Umkehrung der Y-Achse
    client.println("    ctx.lineTo(x, y);"); // Linie zum nächsten Punkt ziehen");
    client.println("  }");
    client.println("  ctx.strokeStyle = 'rgba(75, 192, 192, 1)';");
    client.println("  ctx.lineWidth = 2;");
    client.println("  ctx.stroke();"); // Linie zeichnen");
    client.println("}");

    // Funktion zum Aktualisieren des Diagramms
    client.println("function updateChart(volume) {");
    client.println("  const now = new Date().toLocaleTimeString();");
    client.println("  timeData.push(now);");
    client.println("  volumeData.push(volume);");
    client.println("  if (timeData.length > 10) { // Behalte nur die letzten 10 Werte");
    client.println("    timeData.shift();");
    client.println("    volumeData.shift();");
    client.println("  }");
    client.println("  drawChart();"); // Diagramm neu zeichnen");
    client.println("}");

    // Funktion zum Abrufen des Volumenflusses
    client.println("function fetchVolumeFlow() {");
    client.println("  fetch('/getVolumeFlow') ");
    client.println("    .then(response => response.json())");
    client.println("    .then(data => {");
    client.println("      updateChart(data.volume);");
    client.println("    });");
    client.println("}");
...
void loop() {
                ...
                if (c == '\n') { // If the byte is a newline character
                    ...
                    if (currentLine.length() == 0) {
                        ...
                        if (currentLine.startsWith("GET /setVolumeFlow?value=")) {
                            ...
                            Serial.println("Berechnete Motor steps per Second:");
                            Serial.println(motorSteps);
                            myStepper.setMaxSpeed(motorSteps); // Setzen Sie die neue Geschwindigkeit für den Motor}
                        }

                        if (currentLine.startsWith("GET /getVolumeData HTTP/1.1")) {
                            Serial.println();
                            ...
                            // client.println();
                            // client.print("{\"volume\": ");
                            // client.print(currentVolume); // Aktuellen Volumenstand zurückgeben
                            // client.println("}");
    ...
    if (motorRunning) {
        // Bewege den Motor um 1 Schritt
        myStepper.moveTo(stepCount + 1); // Setze das Ziel um 1 Schritt vorwärts
        myStepper.run(); // Führe die Bewegung aus

        stepCount += 1; // Erhöhe die Schrittzahl
