...
void sendHTML(WiFiClient &client){
    ...
    client.println("const canvas = document.getElementById('volumeChart');");
    client.println("const ctx = canvas.getContext('2d');");

    client.println("function drawChart() {");
    client.println("  ctx.clearRect(0, 0, canvas.width, canvas.height);");

    client.println("  // Achsen");
    client.println("  ctx.strokeStyle = '#000';");
    client.println("  ctx.beginPath();");
    client.println("  ctx.moveTo(40, 10);");
    client.println("  ctx.lineTo(40, 180);");
    client.println("  ctx.lineTo(390, 180);");
    client.println("  ctx.stroke();");

    client.println("  if (volumeData.length < 2) return;");

    client.println("  const maxVolume = Math.max(...volumeData, initialVolume);");
    client.println("  const scaleX = 330 / (volumeData.length - 1);");
    client.println("  const scaleY = 160 / maxVolume;");

    client.println("  ctx.strokeStyle = 'rgba(75, 192, 192, 1)';");
    client.println("  ctx.beginPath();");

    client.println("  volumeData.forEach((v, i) => {");
    client.println("    const x = 40 + i * scaleX;");
    client.println("    const y = 180 - v * scaleY;");
    ...
    client.println("function updateChart(v) {");
    client.println("  timeData.push(new Date().toLocaleTimeString());");
    client.println("  volumeData.push(v);");

    client.println("  if (volumeData.length > 60) {");
    client.println("    volumeData.shift();");
    client.println("    timeData.shift();");
    client.println("  }");

    client.println("  drawChart();");
    client.println("}");
