...
void readSensors() {
    ...
    airSensorValue = analogRead(airSensorPin);
    ...
    heliumSensorValue = analogRead(heliumSensorPin);
...
void handleEmptyButton(){
    int emptyButtonReading = digitalRead(EMPTY_BUTTON_PIN);
    if (emptyButtonReading == LOW && empty_button_status != 0) { 
        empty_button_status = 0;
        motorRunning = false;
        Serial.println("Empyt Button gedrückt -> Motor gestoppt, Start gesperrt");

    } else if(emptyButtonReading == HIGH && empty_button_status != 1){  // Button losgelassen
        empty_button_status = 1;
        Serial.println("Empty Button losgelassen -> Start wieder erlaubt");
    }
}

void handleFullButton(){
    int fullButtonReading = digitalRead(FULL_BUTTON_PIN);
    if (fullButtonReading == LOW && full_button_status != 0) { 
        full_button_status = 0;
        motorRunning = false;
        Serial.println("Full Button gedrückt -> Motor gestoppt, Start gesperrt");

    } else if(fullButtonReading == HIGH && full_button_status != 1){  // Button losgelassen
        full_button_status = 1;
        Serial.println("Full Button losgelassen -> Start wieder erlaubt");
    }
}
...
void sendHTML(WiFiClient &client){
    ...
    client.println("<script>");
    client.print("var emptyButtonStatus = "); 
    client.print(empty_button_status ? 1: 0); 
    client.println(";");
    client.print("var fullButtonStatus = "); 
    client.print(full_button_status ? 1: 0); 
    client.println(";");
    client.println("function startMotor() {");
    client.println("    console.log('EmptyButtonstatus:', emptyButtonStatus);"); // Debug
    client.println("    console.log('FullButtonstatus:', fullButtonStatus);"); // Debug
    ...
    client.println("    if(emptyButtonStatus == 0){");
    ...
    client.println("        }");
    client.println("    }");
    client.println("    else if (fullButtonStatus == 0){");
    client.println("        if (!(num > 0)) {");
    ...
    // client.println("  fetch('/emptyButtonStatus').then(r=>r.text()).then(s=>{");
    // client.println("    emptyButtonStatus = parseInt(s);");
    // client.println("    console.log('ButtonstatusInterval:', emptyButtonStatus);"); // Debug
    // client.println("    if(emptyButtonStatus == 0){");
    // client.println("      document.getElementById('errorMsg').innerText = 'Empty Button pressed, Motor stopped!';");
    // client.println("    } else if (emptyButtonStatus == 1) {");
    // client.println("      document.getElementById('errorMsg').innerText = '';"); 
    // client.println("    }");
    // client.println("  });");
    // client.println("}");
    ...
    // // Buttonstatus regelmäßig abfragen
    // client.println("function checkFullButton(){");
    // client.println("  fetch('/empytButtonStatus').then(r=>r.text()).then(s=>{");
    // client.println("    fullButtonStatus = parseInt(s);");
    // client.println("    console.log('ButtonstatusInterval:', fullButtonStatus);"); // Debug
    // client.println("    if(fullButtonStatus == 0){");
    // client.println("      document.getElementById('errorMsg').innerText = 'Full Button pressed, Motor stopped!';");
    // client.println("    } else if (fullButtonStatus == 1) {");
    // client.println("      document.getElementById('errorMsg').innerText = '';"); 
    // client.println("    }");
    // client.println("  });");
    // client.println("}");
    // //client.println("setInterval(checkFullButton, 10000);"); // jede Sekunde prüfen
...
void processRequest(WiFiClient &client, String currentLine){
    ...
    // if (currentLine.startsWith("GET /emptyButtonStatus")) {
    //     client.println("HTTP/1.1 200 OK");
    //     client.println("Content-Type: text/plain");
    //     client.println("Connection: close");
    //     client.println();
    ...
    // if (currentLine.startsWith("GET /fullButtonStatus")) {
    //     client.println("HTTP/1.1 200 OK");
    //     client.println("Content-Type: text/plain");
    //     client.println("Connection: close");
    //     client.println();
    //     client.println(full_button_status);   // sendet nur "0" oder "1"
    //     Serial.println(full_button_status);
    //     delay(5);
    //     client.stop();
    //     return;
    // }
