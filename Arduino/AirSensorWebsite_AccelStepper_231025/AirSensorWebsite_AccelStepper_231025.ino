...
void loop() {
    ...
    if (digitalRead(BUTTON_PIN) == LOW && button_status != 0) { // Check again after debounce
        // if (button_status != 0) {
        button_status = 0;
        motorRunning = false;

        Serial.println("Button gedrückt -> Motor gestoppt, Start gesperrt");
       // while (digitalRead(BUTTON_PIN) == LOW); // Kurze Verzögerung, um die Schaltlogik abzukapseln
        // }
    } else if(digitalRead(BUTTON_PIN) == LOW && button_status != 1){  // Button losgelassen
        //if (button_status != 1) {
        button_status = 1;
            ...
            if (client.available()) { // If there's bytes to read from the client,
                ...
                if (c == '\n') { // If the byte is a newline character
                    ...
                    if (currentLine.length() == 0) {
                        ...
                        client.println("<div id='buttonMsg' style='color:red'></div>");
                        ...
                        // client.println("    if(parseInt(s) === 0){");
                        // client.println("      document.getElementById('buttonMsg').innerText='Button gedrückt!!'; return;");
                        // client.println("    }");
                        ...
                        client.println("            document.getElementById('errorMsg').innerText = 'Bitte nur positive Zahlen eingeben.';");
                        ...
                        //  // Polling für Button-Warnung
                        // client.println("function checkButton(){");
                        // client.println("  fetch('/buttonStatus').then(r=>r.text()).then(s=>{");
                        // client.println("    if(parseInt(s)===0){ document.getElementById('errorMsg').innerText='Button gedrückt!!'; }");
                        // client.println("    else{ document.getElementById('errorMsg').innerText=''; }");
                        // client.println("  });");
                        // client.println("}");
                        // client.println("setInterval(checkButton,500);"); // alle 500ms prüfen
                        ...
                        if (currentLine.startsWith("GET /getMotorStatus")) {
                            ...
                            client.println("}");
                            client.stop();
                            Serial.println("Motor status ausgeleses");
