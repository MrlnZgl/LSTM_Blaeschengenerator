...
int empty_sensor_status = 1;
int full_sensor_status = 1;
int distance_status = 1;
...
void handleDistanceSensor(){
    ...
    if (fullLevelReading == LOW && full_sensor_status != 0) { 
        ...
        motorRunning = false;
        Serial.println("Spritzenfüllstand hoch -> Motor gestoppt, Start gesperrt");

    } else if(fullLevelReading == HIGH && full_sensor_status != 1){  // Button losgelassen
        full_sensor_status = 1;
        Serial.println("Spritzenfüllstand niedrig genug -> Start wieder erlaubt");
    }
}
...
void handleRequests(WiFiClient &client, String requestLine){
  ...
  if (requestLine.startsWith("GET /getHeliumflow")) {
      ...
      delay(5);
      client.stop();
     // Serial.println("Read Heliumflow");
      return;
  }

  // ------------------------------- handle empty sensor request ---------------------------------
  if (requestLine.startsWith("GET /emptySensorStatus")) {
        client.println("HTTP/1.1 200 OK");
        client.println("Content-Type: text/plain");
        client.println("Connection: close");
        client.println();
        client.println(empty_sensor_status);   // sendet nur "0" oder "1";
        delay(5);
        client.stop();
        return;
    }

    // ------------------------------- handle full sensor request ---------------------------------
  if (requestLine.startsWith("GET /fullSensorStatus")) {
        client.println("HTTP/1.1 200 OK");
        client.println("Content-Type: text/plain");
        client.println("Connection: close");
        client.println();
        client.println(full_sensor_status);   // sendet nur "0" oder "1";
        delay(5);
        client.stop();
        return;
    }
...
void handleClient(WiFiClient &client){
  ...
  client.print("var emptySensorStatus = "); 
  client.print(empty_sensor_status ? 1: 0); 
  client.println(";");
  client.print("var fullSensorStatus = "); 
  client.print(full_sensor_status ? 1: 0); 
  client.println(";");
  client.println("function startMotor() {");
  client.println("    var volumeFlow = document.getElementById('injektionVolumeFlow').value;");
  client.println("    var num = parseFloat(volumeFlow);");
  ...
  client.println("              }");
  client.println("         }");
  client.println("         else if (fullSensorStatus == 0){");
  client.println("             if (!(num > 0)) {");
...
// -------------------- Update fullSensorStatus --------------------
  ...
  client.println("  fetch('/fullSensorStatus').then(r=>r.text()).then(s=>{");
  client.println("    fullSensorStatus = parseInt(s);");
  client.println("    console.log('FullSensorstatusInterval:', fullSensorStatus);"); // Debug
  client.println("    if(fullSensorStatus == 0){");
  client.println("      document.getElementById('errorMsg').innerText = 'Full Sensor reached, Motor stopped!';");
  client.println("    } else if (fullSensorStatus == 1) {");
  client.println("      document.getElementById('errorMsg').innerText = '';"); 
  client.println("    }");
  client.println("  });");
  client.println("}");
  ...
  client.println("}");
  client.println("setInterval(fetchHeliumflow, 6000);"); // Alle 5 Sekunden aktualisieren
  client.println("</script>");
