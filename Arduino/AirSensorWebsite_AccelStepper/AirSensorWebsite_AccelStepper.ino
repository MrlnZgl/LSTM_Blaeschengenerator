...
void loop() {
    ...
    if (buttonReading == LOW && button_status != 0) { // Check again after debounce
        // if (button_status != 0) {
        button_status = 0;
        motorRunning = false;
            ...
            if (client.available()) { // If there's bytes to read from the client,
                ...
                if (c == '\n') { // If the byte is a newline character
                    ...
                    if (currentLine.length() == 0) {
                        ...
                        client.println("setInterval(fetchMotorStatus, 10000); ");// Alle 5 Sekunden aktualisieren
                        client.println("setInterval(fetchVolumeFlow, 10000); // Alle 5 Sekunden abrufen");
                        client.println("</script>");
